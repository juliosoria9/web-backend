...
- `ejercicios/practica/bildyapp-api/src/middleware/socket-auth.js:8-10` — Añado un comentario explicando que los joins se hacen en `index.js` tras el `connection`. El middleware sigue siendo "autentica y carga usuario", nada más.
...
En la práctica, dado que ya añadimos `user:<id>` en `src/index.js:43-49`, el usuario sin compañía aún recibe sus `:ack` privados — sólo se pierde el feed compartido. Es un comportamiento aceptable mientras la UI explique que para colaborar hay que crear/unirse a una compañía.
